				...
				<?php while($lastupdate = $lastupdate_query->fetch()) { ?>
					<a href="./?op=detail&table=<?php echo $home['id']; ?>&id=<?php echo $lastupdate['ID']; ?>">
						<div class="thumbnail">
							...
							<li><?php if (file_exists($filename)) echo '<div class="lastadd"><img src="'.$filename.'" class="lastadd-img" title="'.$lastupdate['TitreVF'].'" alt="affiche" /></div>'; else echo '<div class="lastadd"><img data-src="holder.js/100px165?text=aucune \n image" title="'.$lastupdate['TitreVF'].'" alt="affiche" /></div>'; ?></li>
						</div>
					</a>
